...
try{await openStore();state=await read();input.value=state.drafts[tabID]?.text??'';render();stream.scrollTop=stream.scrollHeight;onExternalChange(async()=>{try{state=await read();render();}catch(e){report(e);}});if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});}catch(e){stream.replaceChildren(el('p','无法打开本机存储，请允许浏览器保存网站数据后重试。','warning'));$('#send-button').disabled=true;report(e);}
